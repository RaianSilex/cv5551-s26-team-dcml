...
class ZedCamera:
    ...
    def __init__(self, resolution=sl.RESOLUTION.HD2K, fps=15, exposure=20):
        ...
        self._thread.start()
        while self._image is None or self._point_cloud is None or self._depth is None:
            time.sleep(0.1)
    ...
    def _update(self):
        while self._running:
            ...
            if self._zed.grab(self._runtime_parameters) == sl.ERROR_CODE.SUCCESS:
                ...
                self._zed.retrieve_measure(self._measure_depth, sl.MEASURE.DEPTH)
                self._zed.retrieve_measure(self._measure_XYZ, sl.MEASURE.XYZ)

                with self._lock:
                    self._image = self._image_mat.get_data().copy()
                    self._depth = self._measure_depth.get_data().copy()
